...
class Service:
    ...
    def add(self, url: str, group_id: int | float) -> None:
        task = self.__task_provider.get_task(start_date=datetime.datetime.now(), url=url, chat_id=group_id,
                                             max_flat_number=7)
        self.__cron.add(task_id=str(group_id), fn=task, trigger='interval', minutes=45)
    ...
    def retry_tasks(self):
        ...
        if len(all_jobs):
            for job in all_jobs:
                task = self.__task_provider.get_task(max_flat_number=7, url=job.Url, chat_id=job.GroupId)
                self.__cron.add(task_id=str(job.GroupId), fn=task, trigger='interval', minutes=45)
